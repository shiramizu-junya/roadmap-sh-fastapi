# FastAPI 実践型教材 生成プロンプト（v10 / MySQL・Docker・Alembic + roadmap.sh プロジェクト連動）
...
>
> **v10 の変更点**（CLI の扱いを点検した結果）
> 15. §4.2.1 に**ルール6**を追加。なぜなぜ③で挙げた**代償に手当てがあるなら、回収先を必ず書く**（代償を挙げて言いっぱなしにしない）。
> 16. §2.4 の「今回入れないもの」に **`fastapi` CLI** を追加（`uvicorn` と機能が重なるため）。ルート一覧の確認は **P1-8 で扱う**ことにした。
...
6. **③で挙げた代償に、この教材内で手当てがあるなら、回収先のステップを必ず書く。**
   「〜を失う」で終わらせない。手当てが無いなら「**この教材では手当てしない**」と明示する

> **6 を足した理由**: 代償を挙げただけで終わると、読者には「この設計には弱点がある」という
